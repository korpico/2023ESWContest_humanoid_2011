    ...
    while True:
        ...
        if len(cnts) > 0:
            ...
            if Area > min_area[now_color]:
                ...
                rect_point = (x4 + w4 / 2, y4 + h4 / 2)  # 사각형 중앙 좌표
                print("rect_point : ( ", rect_point, " )")

                if rect_point[1] < (H_View_size / 1.7):  
                    # 중앙 즈음으로 판단할 범위를 넓게 잡음. 
                    # 휴머노이드 수치값들 측정으로 line 518과 연계되게 값 수정 필요
                    
                    TX_data(serial_port, 21)  #Up_Head

                elif rect_point[1] > (H_View_size / 2.2):  
                    TX_data(serial_port, 31)  #Down_Head
                ...
                # 공의 넓이 != 임계값 내의 범위
                if certain_area_1 > Area :  
                    TX_data(serial_port, 41)  # Go. # 전송값: 임의. 수정 필요
                elif Area > certain_area_2:
                    TX_data(serial_port, 42)  #뒤로 Go
                    
                if (required_area[0] - 2 < rect_point[0] < required_area[0] + 2) and \
                   (required_area[1] - 2 < rect_point[1] < required_area[1] + 2):
                        TX_data(serial_port, 31) # 각도 보내라고 시킴
                        readRX = RX_data(serial_port) # 각도 받기
                        distance = cal_distance(height, readRX)
                        if 13 < distance and distance < 15:
                           TX_data(serial_port, 50) # 공 치셈
                           
                elif (required_area[0] - 2 > rect_point[0]) and \
                     (required_area[1] - 2 > rect_point[1]):
                        TX_data(serial_port, 44) # Move_Left
                        TX_data(serial_port, 31) # Down_Head
                
                else:
                    TX_data(serial_port, 45) # Move_Right
                    TX_data(serial_port, 21) # Up_Head
            ...
            ## 아무것도 hsv영역에 들어오지않았을때
            # TX_data(serial_port, 31) ## 머리 천천히 아래
            # TX_data(serial_port, 21) ## 머리 천천히 위
