...
robo_height = 0.35 #로봇의 높이 35 cm
H_degree = 80 #머리 각도
ball_distance = 0 # 공 거리
...
if __name__ == '__main__':
    ...
    while True:
        ...
        center = None
        # TX_data(serial_port, 33) # head 60 degree
        # print('head 60')
        ...
        if len(cnts) > 0:
            ...
            if Area > min_area[now_color]:
                ...
                # if rect_point[1] < (H_View_size / 1.9):  
                ...
                # elif rect_point[1] > (H_View_size / 2.1):  
                ...
                # cv2.line(frame, (x4 + w4 / 2, y4 + h4 / 2), (x4 + w4 / 2, y4 + h4 / 2), (255, 0, 0), 5, 4, 1)
                ...
                #     time.sleep(0.5)
                '''
                if abs(required_area[0] - rect_point[0]) <= 10:
                        # TX_data(serial_port, 29) # robot head 80 degree
                        # readRX = RX_data(serial_port) # 각도 받기
                        ball_distance = cal_distance(robo_height, 80)
                        if 13 < ball_distance and ball_distance < 15:
                            TX_data(serial_port, 2) # 공 치셈
                            print('golf shot')
                           
                elif (required_area[0] - rect_point[0]) > 10:
                        TX_data(serial_port, 14) # Move_Left
                        # TX_data(serial_port, 31) # Down_Head
                        print('go left')
                
                else:
                    TX_data(serial_port, 13) # Move_Right
                    #TX_data(serial_port, 21) # Up_Head
                    print('go right')
                    '''
